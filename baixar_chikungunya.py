...
Os arquivos extraídos são salvos no formato:
bruto/CHIKBR<AA>.csv
...
def baixar_https(url: str, destino_zip: Path) -> None:
    """
    Baixa o ZIP por HTTPS em fluxo, sem carregar
    o arquivo inteiro na memória.
    """

    temporario = destino_zip.with_suffix(
        destino_zip.suffix + ".part"
    )
    ...
    try:
        ...
        with urlopen(
            ...
            if status != 200:
                raise RuntimeError(
                    f"Resposta HTTP inesperada: {status}"
                )
...
def baixar_ftp(
    ...
    try:
        ...
        if (
            ...
            raise RuntimeError(
                "O FTP retornou um arquivo vazio."
            )
...
def extrair_csv_seguro(
    ...
    O arquivo interno do ZIP é renomeado para
    CHIKBR<AA>.csv, garantindo compatibilidade
    com processar_ride.py mesmo que o nome interno
    mude.
    """

    saida_csv = destino / nome_csv(ano)

    temporario = saida_csv.with_suffix(
        saida_csv.suffix + ".part"
    )
        ...
        with zipfile.ZipFile(
            ...
            if arquivo_corrompido is not None:
                ...
                raise zipfile.BadZipFile(
                    "Arquivo corrompido dentro do ZIP: "
                    f"{arquivo_corrompido}"
                )
            ...
            if len(membros_csv) != 1:
                ...
                nomes = [
                    membro.filename
                    for membro in membros_csv
                ]

                raise RuntimeError(
                    "Era esperado exatamente um CSV "
                    "no ZIP, mas foram encontrados "
                    f"{len(membros_csv)}: {nomes}"
                )
...
def baixar_com_retry(
    ...
    """
    Baixa, valida e extrai um ano,
    repetindo em caso de falha.
    """
    ...
    for tentativa in range(
        ...
        try:
            ...
            if tentativa == TENTATIVAS:
                ...
                raise RuntimeError(
                    "Falha definitiva ao baixar "
                    f"o ano {ano} pela fonte {fonte}."
                ) from erro
...
def main() -> None:
    ...
    # -------------------------------------------------------
    # Confere se todos os CSVs existem
    # -------------------------------------------------------
